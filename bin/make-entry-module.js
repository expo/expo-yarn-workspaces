...
function makeEntryModule(projectPath) {
  ...
  if (!packageJson) {
    debug(
      `The project doesn't have a package.json file; will not make a main module`,
    );
    return;
  }
  ...
  if (!mainModule.endsWith(".js")) {
    console.error(
      `The specified main module must be a file path ending with .js`,
    );
    return;
  }
  ...
  if (fs.existsSync(mainModulePath)) {
    ...
    if (!mainModuleCode.includes("@generated")) {
      debug(
        `The main module at %s already exists; will not make a main module`,
        mainModule,
      );
  ...
  debug(
    `Ensuring that the directory for the main module exists: %s`,
    mainModuleDirectory,
  );
  ...
  const template = fs.readFileSync(
    path.join(__dirname, "AppEntry.template.js"),
    "utf8",
  );
  ...
  let relativeProjectPath = path.relative(
    fs.realpathSync(mainModuleDirectory),
    projectPath,
  );
